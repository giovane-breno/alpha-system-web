...
import { Close, DataArraySharp, Delete, Done, Edit, ExpandLess, ExpandMore, Info, MoreVert, Visibility } from '@mui/icons-material';
import MagnifyingGlassIcon from '@heroicons/react/24/solid/MagnifyingGlassIcon';
import { useState } from 'react';
import { DeleteRole, FindRole, UpdateRole } from 'src/services/HumanResourceService';
...
export const ViewModal = ({ id, refreshState, setRefreshState }) => {
    ...
    const handleOpen = async () => {
        try {
            ...
            setOpen(true);
            setLoading(true);
            const { data, status } = await FindRole(id);
            if (status === 'success') {
                setForm({
                    name: data.name,
    ...
    const saveForm = async () => {
        try {
            const { status } = await UpdateRole(id, form);
            if (status === 'success') {
                enqueueSnackbar('Função atualizada com sucesso!', { variant: 'success', position: 'top-right' });
                setError("");
                    ...
                    <Box sx={small}>
                        ...
                        <Box sx={{ p: 2 }}>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    <TextField fullWidth label="Salário Base" type='number' variant="outlined" value={form.base_salary} InputProps={{ readOnly: isDisabled }} onChange={e => { setForm({ ...form, base_salary: e.target.value }) }} error={!!(error?.base_salary)} helperText={error?.base_salary} />
                                </Grid>
                            </Grid>
                        </Box>
                        <Divider />
