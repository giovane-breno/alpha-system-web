...
import { DeleteDivision, FindDivision, UpdateDivision } from 'src/services/HumanResourceService';
...
export const ViewModal = ({ id, refreshState, setRefreshState }) => {
    ...
    const handleOpen = async () => {
        try {
            ...
            setOpen(true);
            setLoading(true);
            const { data, status } = await FindDivision(id);
            if (status === 'success') {
                setForm({
                    name: data.name,
                    bonus: data.bonus,
    ...
    const saveForm = async () => {
        try {
            const { status } = await UpdateDivision(id, form);
            if (status === 'success') {
...
export const DeleteModal = ({ id, refreshState, setRefreshState }) => {
    ...
    const deleteForm = async () => {
        try {
            const { status } = await DeleteDivision(id);
            if (status === 'success') {
